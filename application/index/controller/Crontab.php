...
//        ]);
   ...
   //----------------------------利息宝---------------------------------
    ...
    public function lxb_jiesuan()
    {
     ...
     /**
     * @地址      lxb_jiesuannew
     * @说明      每天计算理财收益，写入xy_balance_log,不实际到账。等提现时候再到账
     * @说明      域名为  http://域名/index/crontab/lxb_jiesuannew
     * @参数      @参数 @参数
     */
    
  public function lxb_jiesuannew()
    {
        $now = time();
        $today= strtotime( date('Y-m-d 00:00:00', time()) );; //小于今天的 凌晨0
        $lxb = Db::name('xy_lixibao')->where('addtime','<',$today)->where('endtime','>',$now)->where('is_qu',0)->where('is_sy',0)->where('type',1)->select();  //利息宝月
        $todayjiesuan=0;
        if ($lxb) {
            foreach ($lxb as $item) {
                $lixibao = Db::name('xy_lixibao_list')->find($item['sid']);
                $sy = $item['num'] * $lixibao['bili'];
                
                $map['uid']=$item['uid'];
                $map['oid']=$item['id'];
                $map['type']=23;
                $checkjiesuan=Db::name('xy_balance_log')->where($map)->where('addtime','>',$today)->find();
                if(!$checkjiesuan){
                Db::name('xy_users')->where('id',$item['uid'])->setInc('lixibao_dj_balance',$sy);  //利息宝预计收益增加 -
                    $res1 = Db::name('xy_balance_log')->insert([
                    //记录返佣信息
                    'uid'       => $item['uid'],
                    'oid'       => $item['id'],
        ...
        return json(['code'=>1,'info'=>'理财中'.count($lxb).'本次结算'.$todayjiesuan]);
    }
