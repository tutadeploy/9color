...
require.config({
  ...
  shim: {
    ...
    "jquery.autocompleter": {
      deps: ["jquery", "css!" + baseRoot + "plugs/jquery/autocompleter.css"],
    },
  },
});
...
$(function () {
  ...
  $.msg = new (function (that) {
    ...
    this.auto = function (ret, time) {
      ...
      return parseInt(ret.code) === 1
        ? this.success(msg, time, function () {
  ...
  $.form = new (function (that) {
    ...
    this.reInit = function ($dom) {
      ...
      $dom
        .find("[data-file]:not([data-inited])")
        .map(function (index, elem, $this, field) {
          ($this = $(elem)), (field = $this.attr("data-field") || "file");
          if (!$this.data("input"))
            $this.data("input", $('[name="' + field + '"]').get(0));
    ...
    this.href = function (url, obj) {
      if (url !== "#") {
        ...
        $(
          '[data-menu-node^="' +
            obj.getAttribute("data-menu-node") +
            '-"][data-open!="#"]:first'
        ).trigger("click");
      }
    };
    ...
    this.load = function (
      ...
      $.ajax({
        ...
        error: function (XMLHttpRequest) {
          if (XMLHttpRequest.responseText.indexOf("exception") > -1)
            layer.open({
              ...
              success: function ($element, index) {
                try {
                  ...
                  $element
                    .find("iframe")[0]
                    .contentWindow.document.write(XMLHttpRequest.responseText);
                  $element
                    .find(".layui-layer-setwin")
                    .css({ right: "35px", top: "28px" })
                    .find("a")
                    .css({ marginLeft: 0 });
          ...
          if (parseInt(XMLHttpRequest.status) === 200) {
            ...
            $.msg.tips(
              "E" + XMLHttpRequest.status + " - 服务器繁忙，请稍候再试！"
            );
          }
        },
        success: function (ret) {
          if (
            ...
            callback.call(that, ret) === false
          )
            return false;
          return typeof ret === "object"
            ? $.msg.auto(ret, time || ret.wait || undefined)
            : that.show(ret);
        },
        complete: function () {
          $.msg.close(index);
    ...
    this.open = function (url, data, callback, loading, tips) {
      this.load(
        ...
        function (ret) {
          return (
            typeof ret === "object" ? $.msg.auto(ret) : that.show(ret), false
          );
        },
        loading,
        tips
  ...
  $.menu = new (function (that) {
    ...
    this.getUri = function (uri) {
      ...
      uri =
        uri.indexOf(window.location.host) > -1
          ? uri.split(window.location.host)[1]
          : uri;
      return (uri.indexOf("#") > -1 ? uri.split("#")[1] : uri).split("?")[0];
    };
    // 通过URI查询最有可能的菜单NODE
    this.queryNode = function (url, node) {
      node = node || location.href.replace(/.*spm=([\d\-m]+).*/gi, "$1");
      if (!/^m-/.test(node)) {
        var $menu = $(
          '[data-menu-node][data-open*="' + url.replace(/\.html$/gi, "") + '"]'
        );
    ...
    this.parseUri = function (uri, obj) {
        ...
        for (var i in attrs)
          if (attrs[i].indexOf("=") > -1) {
            ...
            if (typeof tmp[0] === "string" && tmp[0].length > 0) {
              params[tmp[0]] = decodeURIComponent(
                tmp[1].replace(/%2B/gi, "%20")
              );
            }
          }
      }
      ...
      if (typeof params.spm !== "string") {
        params.spm =
          (obj && obj.getAttribute("data-menu-node")) || this.queryNode(uri);
      }
    ...
    this.listen = function () {
      ...
      (function ($menu, miniClass) {
        ...
        $body
          .on("click", "[data-target-menu-type]", function () {
            $menu.toggleClass(miniClass);
            layui.data("admin-menu-type", {
              key: "type-min",
              value: $menu.hasClass(miniClass),
            });
          })
          .on("resize", function () {
            if ($body.width() > 1000) {
              layui.data("admin-menu-type")["type-min"]
                ? $menu.addClass(miniClass)
                : $menu.removeClass(miniClass);
            } else {
              $menu.addClass(miniClass);
            }
          })
        ...
        $("[data-target-tips]")
          .mouseenter(function () {
            if ($menu.hasClass(miniClass)) {
              $(this).attr(
                "index",
                layer.tips($(this).attr("data-target-tips") || "", this)
              );
            }
          })
          .mouseleave(function () {
      ...
      window.onhashchange = function (hash, node) {
        ...
        if (/^m-/.test(node)) {
          var $all = $("a[data-menu-node]").parent(),
            tmp = node.split("-"),
            tmpNode = tmp.shift();
          while (tmp.length > 0) {
            tmpNode = tmpNode + "-" + tmp.shift();
            $all = $all.not(
              $('a[data-menu-node="' + tmpNode + '"]')
                .parent()
                .addClass("layui-this")
            );
          }
          ...
          if (node.split("-").length > 2) {
            var _tmp = node.split("-"),
              _node = _tmp.shift() + "-" + _tmp.shift();
            $("[data-menu-layout]")
              .not(
                $('[data-menu-layout="' + _node + '"]').removeClass(
                  "layui-hide"
                )
              )
              .addClass("layui-hide");
    ...
    return new (function (that) {
      ...
      this.checkInput = function (input) {
        var tag = input.tagName.toLowerCase(),
          need = this.hasProp(input, "required");
        ...
        return this.isRegex(input)
          ? (this.hideError(input), true)
          : this.remind(input);
      };
      // 验证标志
      ...
      this.insertError = function (ele) {
        var $html = $(
          '<span style="padding-right:12px;color:#a94442;position:absolute;right:0;font-size:12px;z-index:2;display:block;width:34px;text-align:center;pointer-events:none"></span>'
        );
        $html.css({
        ...
        $(ele).data("input-info") ||
          $(ele).data("input-info", $html.insertAfter(ele));
      };
      // 表单验证入口
      this.check = function (form, callback) {
        ...
        $(form)
          ...
          .map(function () {
            ...
            for (var e in that.checkEvent)
              if (that.checkEvent[e] === true) {
                $(this)
                  .off(e, this.bindEventMethod)
                  .on(e, this.bindEventMethod);
              }
          });
        ...
        $(form)
          .find("[data-form-loaded]")
          .map(function () {
            $(this).html(
              this.getAttribute("data-form-loaded") || this.innerHTML
            );
            $(this)
              .removeAttr("data-form-loaded")
  ...
  $.fn.uploadFile = function (callback) {
    ...
    var that = this,
      mode = $(this).attr("data-file") || "one";
    this.attr("data-inited", true).attr(
      "data-multiple",
      mode !== "btn" && mode !== "one" ? 1 : 0
    );
  ...
  $.fn.uploadOneImage = function () {
    ...
    var $tpl = $('<a data-file="btn" class="uploadimage"></a>')
      .attr("data-field", name)
      .attr("data-type", type);
    $(this)
      .attr("name", name)
      .after($tpl.data("input", this))
      .on("change", function () {
  ...
  $.fn.uploadMultipleImage = function () {
    ...
    $(this)
      ...
      .on("change", function () {
        ...
        var urls = this.getImageData(),
          srcs = this.value.split("|");
        for (var i in srcs) if (srcs[i]) urls.push(srcs[i]);
        $(this).prevAll(".uploadimage").remove();
        this.value = urls.join("|");
        for (var i in urls) {
          var tpl =
            '<div class="uploadimage uploadimagemtl"><a class="layui-icon margin-right-5">&#xe602;</a><a class="layui-icon margin-right-5">&#x1006;</a><a class="layui-icon margin-right-5">&#xe603;</a></div>';
          var $tpl = $(tpl)
            .attr("data-tips-image", urls[i])
            .css("backgroundImage", "url(" + urls[i] + ")")
  ...
  $body.on("submit", "form.form-search", function () {
    ...
    if ((this.method || "get").toLowerCase() === "get") {
      return (window.location.href =
        "#" + $.menu.parseUri(url + split + $(this).serialize()));
    }
  ...
  $body.on("click", "[data-modal]", function () {
    return $.form.modal(
      ...
      "open_type=modal",
      $(this).attr("data-title") || $(this).text() || "编辑"
    );
  });
  ...
  $body.on("dblclick", "[data-dbclick]", function () {
    $(this)
      .find(this.getAttribute("data-dbclick") || "[data-dbclick]")
      .trigger("click");
  });
  ...
  $body.on("click", "[data-action]", function () {
    ...
    var rule =
      ...
      function (rule, ids) {
        $(
          $this.attr("data-target") || "input[type=checkbox].list-check-box"
        ).map(function () {
    ...
    data["_csrf_"] =
      $this.attr("data-token") || $this.attr("data-csrf") || "--";
    var load = loading !== "false",
      tips = typeof loading === "string" ? loading : undefined;
    if (!$this.attr("data-confirm"))
  ...
  $body.on("blur", "[data-action-blur]", function () {
    var data = {},
      that = this,
      $this = $(this),
      action = $this.attr("data-action-blur");
    ...
    that.callback = function (ret) {
      $this.css(
        "border",
        ret && ret.code ? "1px solid #e6e6e6" : "1px solid red"
      );
      return false;
    };
    data["_csrf_"] =
      $this.attr("data-token") || $this.attr("data-csrf") || "--";
    if (!confirm)
      return $.form.load(action, data, method, that.callback, load, tips, time);
  ...
  $body.on("click", "[data-icon]", function (field, location) {
    ...
    location = window.ROOT_URL + "?s=sgcpj/api.plugs/icon.html&field=" + field;
  ...
  $body
    .on("mouseenter", "[data-tips-text]", function () {
      $(this).attr(
        "index",
        layer.tips($(this).attr("data-tips-text"), this, {
          tips: [$(this).attr("data-tips-type") || 3, "#78BA32"],
        })
      );
    })
  ...
  $body.on("click", "[data-tips-image]", function () {
    $.previewImage(
      this.getAttribute("data-tips-image") || this.src,
  ...
  $.previewPhonePage = function (href, title) {
    var tpl =
      '<div><div class="mobile-preview pull-left"><div class="mobile-header">_TITLE_</div><div class="mobile-body"><iframe id="phone-preview" src="_URL_" frameborder="0" marginheight="0" marginwidth="0"></iframe></div></div></div>';
    layer.style(
      layer.open({
        type: true,
        scrollbar: false,
        ...
        skin: "layui-layer-nobg",
        content: $(
          tpl.replace("_TITLE_", title || "公众号").replace("_URL_", href)
        ).html(),
      }),
      { boxShadow: "none" }
    );
  ...
  $body.on("click", "[data-refresh-captcha]", function (image, verify, uniqid) {
    (image = this),
      (uniqid = this.getAttribute("data-uniqid-field") || "uniqid");
    verify =
      this.getAttribute("data-refresh-captcha") ||
      this.getAttribute("data-verify-field") ||
      "verify";
    $.form.load(
