define(function () {
  return function (element, InitHandler, UploadedHandler, CompleteHandler) {
    ...
    // 检查可以上传的文件后缀
    $.form.load(
      "/sgcpj/api.plugs/check.html",
      { exts: exts, uptype: uptype },
      "post",
      function (ret, options) {
        options = {
          url: ret.data.data.url,
          exts: ret.data.exts,
          acceptMime: ret.data.mime,
          data: ret.data.data,
        };
        if (exts.indexOf("*") > -1)
          delete options.exts, delete options.acceptMime;
    ...
    function renderUploader(options, headers) {
      this.options = {
        ...
        before: function () {
          this.proindex = $.msg.loading(
            "上传进度 <span data-upload-progress>0%</span>"
          );
        },
        progress: function (n) {
